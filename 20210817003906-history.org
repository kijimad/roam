...
|-----------+--------------------------------------------------------------------|
| 概要      | セキュリティSaaSの開発                                               |
| 使用技術   | TypeScript, React, NestJS, GitHub Actions, AWS(ECS, Athena, S3...) |
| 期間      | 2026-02 ~ 現在                                                     |
| チーム人数 | 5名                                                                |
| 担当比率   | Backend 80% / Infra 20%                                            |

- 状況

  拡大フェーズのセキュリティSaaSに参画し、バックエンド中心に拡大に対応するための整備を担当した。

- 課題

  少人数チームで開発されてきたなかで規模が拡大し、トラフィックに対応する必要やより安全性を高めつつ開発速度を上げていく必要があった。

- 行動

  機能開発に加えて、知見を持っている開発改善分野について取り組んだ。

- 安全に並列処理するための根本改修、DBエラーの複数の根本解決と再発防止
- テスト時間を94%削減(11分->40秒)
  - ボトルネックを特定し、最小限の工数で最大の成果を上げた。並列実行でマシンの全CPUを最大限活用できるようになった
  - 開発者1人あたり月約20時間の待ち時間を削減
  - 高速化と同時に、フレーキー率0%
- ビルド時間を80%削減(10分->2分)
  - ボトルネックを特定し、最小限の工数で最大の成果を上げた。CIでのDockerキャッシュの最適化とイメージサイズの削減で高速化した
  - チームで月約14時間(17時間->3時間)の待ち時間を削減
- データベースのリファクタリング
  - フィールドやテーブルについて、安全に変更/削除した
- マイグレーションフローの整備とガードレール整備
- AWS CDKの不正な状態の回復とガードレール整備
  - 各環境でコードと異なる状態を示していたので、すべて調査のうえ差分がない状態にし、クリーンな状態が保たれるようにした
...
- 自作RPG [[https://github.com/kijimaD/ruins][ruins]]の機能追加
