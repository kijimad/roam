...
| 期間      | 2024-09 ~ 現在                                                |
| チーム人数 | フルスタック2名                                                 |
| 担当比率   | バックエンド70%、フロントエンド20%、 インフラ10%                   |

- 状況

  地図系SaaSのリリース後のフェーズで開発に参画し、主にバックエンド開発者として、フロントエンドも含めた新機能の開発や変更、リファクタリングに従事した。

- 課題

  顧客要件の変更に伴い、権限構造の根本的な改修が必要となった。
  稼働中のシステムへの影響を最小限に抑えつつ、将来の権限要件にも対応できる設計が求められた。変わりやすい部分に対してコードの変更が不要になるように変更した。
...
- 行動
  ...
  3. コードに変更を加える前に関連する箇所のガードレールの整備をおこなった。
     コードベースの課題に合ったLinterルールの導入、CIによって自動化や作業漏れを検知できるようにした。
     膨大な実装やテストの修正を安全に行えた。

- 成果

  1. 仕様面での疎通ミス、設計段階での考慮もれ、などの手戻りはほとんど発生しなかった。
...
| 期間      | 2022-12 ~ 2024-09                                      |
| チーム人数 | バックエンド1名、3Dフロントエンド5名                       |
| 担当比率   | バックエンド100%                                        |

- 状況

  測量会社におけるブラウザ・ネイティブ対応の3D GISの新規開発に参画し、バックエンド開発者として機能開発に従事した。
...
- 行動
  ...
  2. 短くフォーカスした会議を多く設定して接触機会を増やしたり、食事に誘って心理的ハードルを下げた。
     ...
     OpenAPIを用いたスキーマ駆動開発をリードし、他社チーム間の仕様策定を円滑化した。
...
| 期間      | 2021-12 ~ 2022-08                          |
| チーム人数 | バックエンド5名                              |
| 担当比率   | バックエンド50%、フロントエンド20%、インフラ30% |

- 状況
  開始から数年が経過したサービスにおいて、主にバックエンド開発者として、フロントエンド、インフラも含めた機能開発やリファクタリング、インフラ改修に従事した。
...
- 行動
  ...
  2. テストの「部長」として業務の一定割合をコード品質強化に割り当てた。
     ...
     関係者への調整を経て、数年分のレガシーコードを大量に削除した。
...
業界よりも、自分自身が理解できる課題と難易度の高い技術課題に取り組むことを重視している。
