...
これはプログラム関連の文書シリーズである。この文書の目的は主に3つある。
...
#+begin_src bash :results raw :exports value
  file_changed_count_day() {
      ...
      count=`git log --diff-filter=A --since="$datestr_past" --until="$datestr_future" --name-only --pretty=format: | grep -e ".*org" | sort -u | wc -l`
  ...
  echo
  echo "追加ファイル"
  echo "2年間" `git log --diff-filter=A --since="2 year ago" --name-only --pretty=format: | grep -e ".*org" | sort -u | wc -l`
  echo "1年間" `git log --diff-filter=A --since="1 year ago" --name-only --pretty=format: | grep -e ".*org" | sort -u | wc -l`
  echo "1ヶ月間" `git log --diff-filter=A --since="1 month ago" --name-only --pretty=format: | grep -e ".*org" | sort -u | wc -l`
  echo "1週間" `git log --diff-filter=A --since="1 week ago" --name-only --pretty=format: | grep -e ".*org" | sort -u | wc -l`
...
#+caption: ポモドーロ統計の図
 ...
 <body>
 <div id="calendar_basic" style="width: 100%; max-width: 1000px;"></div>
 </body>
 #+END_EXPORT
** This Week by Day
今週の記録。
...
- プロジェクトはステータスを追跡する
** CLOSE 不要なファイルを消す・分割する
CLOSED: [2026-05-10 Sun 12:03]
